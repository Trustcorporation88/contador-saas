import { BadgeAlert, CreditCard, Pencil, Wallet } from 'lucide-react';
...
export default function ContasPagarList({ contas, loading, onEdit, onPagar, onCancelar }: ContasPagarListProps) {
  return (
    <div className="table-container">
      <table className="min-w-full divide-y divide-gray-200">
        ...
        <tbody className="divide-y divide-gray-100 bg-white">
          {loading && (
            <tr>
              <td colSpan={7} className="px-4 py-12 text-center text-sm text-gray-400">Carregando contas a pagar...</td>
            </tr>
          )}
          ...
          {!loading && contas.length === 0 && (
            <tr>
              <td colSpan={7} className="px-4 py-16 text-center">
                ...
                <p className="text-sm text-gray-500">Nenhuma obrigação encontrada.</p>
              </td>
            </tr>
          )}
          ...
          {contas.map((conta) => (
            <tr key={conta.id} className="hover:bg-gray-50 transition-colors">
              ...
              <td className="px-4 py-3 text-sm text-gray-700">
                ...
                <p className="text-xs text-gray-500">{conta.fornecedor_cnpj || 'CNPJ não informado'}</p>
              </td>
              <td className="px-4 py-3 text-sm text-gray-700">
              ...
              <td className="px-4 py-3">
                <div className="flex justify-end gap-2">
                  <Button type="button" variant="ghost" size="sm" icon={<Pencil className="h-4 w-4" />} onClick={() => onEdit(conta)} disabled={conta.status === 'pago' || conta.status === 'cancelado'}>
                    Editar
                  </Button>
                  <Button type="button" variant="secondary" size="sm" icon={<CreditCard className="h-4 w-4" />} onClick={() => onPagar(conta)} disabled={conta.status === 'pago' || conta.status === 'cancelado'}>
                    Pagar
                  </Button>
                  <Button type="button" variant="ghost" size="sm" icon={<BadgeAlert className="h-4 w-4" />} onClick={() => onCancelar(conta)} disabled={conta.status === 'cancelado'}>
                    Cancelar
                  </Button>
                </div>
              </td>
